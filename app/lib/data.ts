...
export async function fetchCardData() {
  try {
    ...
    const numberOfInvoices = Number(rows[0].count ?? '0');

    const { rows: customerRows } = await pool.query(`SELECT COUNT(*) FROM customers`);
    const numberOfCustomers = Number(customerRows[0].count ?? '0');

    const { rows: invoiceStatusRows } = await pool.query(
      `SELECT SUM(CASE WHEN status = 'paid' THEN amount ELSE 0 END) AS "paid", SUM(CASE WHEN status = 'pending' THEN amount ELSE 0 END) AS "pending" FROM invoices`
    );
